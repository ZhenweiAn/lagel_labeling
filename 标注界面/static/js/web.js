...
function collect_tagging(){
    if(curr_page == 1){
        ...
        for(i in yg_suqiu){
            tag_page_7[i] = parseInt($("input[name='input_7_"+i+"']:checked").val());
        }
    }else if(curr_page == 8){
...
function submit_tagging(){
    ...
    var data = {
        ...
        "rel_defen_ap": strArray_to_2dArray(tag_page_4),//原告诉求被告zhengju
        "rel_ap_Ar": strArray_to_2dArray(tag_page_5),
        "rel_defen_ar": strArray_to_2dArray(tag_page_6),
    ...
    $.ajax({
        ...
        success:function(msg){
            ...
            for(i in msg){
                ...
                var chain = new Array();
                if(yg_e != -1){
                    chain.push("原告证据"+(yg_e+1));
                }
                if(yg_s != -1){
                    chain.push("原告诉求"+(yg_s+1));
                }
                if(bg_e != -1){
                    chain.push("被告证据"+(bg_e+1));
                }
                if(bg_b != -1){
                    chain.push("被告辩称"+(bg_b+1));
                }
                chain = chain.join("-");
...
$(document).ready(function(){
    ...
    $("#page_7").click(function(){
        ...
        for(i in yg_suqiu){
            html = "<li>" + yg_suqiu[i]
                   + '<div class="col-sm-12">'
                   + '<input type="radio" name="input_7_'+i+'" value="0"> 支持&nbsp;' 
                   + '<input type="radio" name="input_7_'+i+'" value="1"> 部分支持&nbsp;'
                   + '<input type="radio" name="input_7_'+i+'" value="2"> 驳回 '  
                   + "</div></li>";
            $("#ol_right").append(html);
